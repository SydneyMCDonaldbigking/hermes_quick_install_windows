...
```powershell
# 全局常量 - 在所有脚本中使用
[string]$WSL_DISTRO_NAME = "Ubuntu-22.04"  # 统一使用此名称
...
function Test-HermesConnection {
    ...
    foreach ($ip in $ips) {
        ...
        try {
            $response = Invoke-WebRequest -Uri "http://${ip}:8000/api/health" `
...
wsl -d Ubuntu-22.04 -u root -e bash -lc "cd ~/.hermes && ..."
```
...
- [ ] **关键**: 验证 WSL2 发行版名称统一 (`Ubuntu-22.04`)
...
- [ ] **发行版统一**: 全局变量 `$WSL_DISTRO_NAME = "Ubuntu-22.04"` 在所有脚本中使用
